  ...
  begin
      try
       serviceDB.AtualizaPesoBalancaoAnimais(serviceDB.qryVeiculosID.AsString,
        StringReplace(StringReplace(vPesoMedio,'.','',[rfReplaceAll]),',','.',[rfReplaceAll]));
        serviceDB.AtualizaPesoTotalLoteSaida(serviceDB.qryVeiculosID_LOTESAIDA.AsString);
...
begin
 ...
 begin
   SerRel.AbreRelAnimaisVeiculo(servicedb.qryGridLoteSaidaID.AsString,vIdVeiculoEra);
   if not serRel.qryRelAnimalVeiculo.IsEmpty then
    serRel.ppRepRelAnimalVeiculo.Print
   else
   begin
