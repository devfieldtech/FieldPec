...
uses
  ...
  FireDAC.Comp.Client, ComPort,System.Threading, FMX.EditBox, FMX.SpinBox,
...
type
   TfrmMovPesagemAnimal = class(TForm)
    ...
    public
     ...
     vIdRebanhoDestinoNovo,vIdRebanhoDestinoInsert:string;
...
begin
   ...
   begin
     Clear;
    Add('SELECT ap.*,cu.CAPACIDADE,cu.LOTACAO,cu.id idCurral FROM APARTACAO AP');
     ...
     begin
      ...
      begin
        ...
        GridApartacao.Cells[1,i]     := vQryAux.FieldByName('PESO_MINIMO').AsString;
        GridApartacao.Cells[2,i]     := vQryAux.FieldByName('PESO_MAXIMO').AsString;
        GridApartacao.Cells[3,i]     := vQryAux.FieldByName('ID').AsString;
        GridApartacao.Cells[4,i]     := vQryAux.FieldByName('CAPACIDADE').AsString;
        GridApartacao.Cells[5,i]     := vQryAux.FieldByName('LOTACAO').AsString;
        GridApartacao.Cells[6,i]     := vQryAux.FieldByName('idCurral').AsString;
        cbxCurralPasto.Items.AddObject(vQryAux.FieldByName('CURRAL_PASTO').AsString,TObject(vQryAux.FieldByName('ID_CURRAL_PASTO').AsInteger));
        inc(i);
        vQryAux.Next;
      end;
      ...
      begin
        ...
        begin
          ...
          case frmPrincipal.vConfirma of
            ...
            begin
                try
                 ServiceConf.CriarLoteNutricional(vIdLocal,FormatDateTime('mm/dd/yyyy',date));
...
var
 vTipoCurral,vBloqueio,vPesoEntrada,vDataEntrada:string;
begin
  ...
  try
    ...
    begin
     if vTipoMovi=0 then
       serviceDB.UpdateAnimalReproc(vIDAnimal,vIdLocal,vIdCategoria,vIDRaca,edtIdentificacao1.Text,
        edtIdentificacao2.Text,'','',edtObservacao.Text,vIdRebanhoDestinoInsert,vIdLote,
        vIdContrato,vTipoCurral,intToStr(vTipoMovi),frmPrincipal.vIdBaseCampo,
        DateTimeToStr(now),vBloqueio,cbxDestinoAnimal.Selected.Text);
     if vTipoMovi=1 then
      serviceDB.UpdateAnimalReproc(vIDAnimal,vIdLocal,vIdCategoria,vIDRaca,edtIdentificacao1.Text,
    ...
    if frmPrincipal.vMov=5 then
    begin
      serviceDB.UpdateAnimalReproc(vIDAnimal,vIdLocal,vIdCategoria,vIDRaca,
      edtIdentificacao1.Text,
      edtIdentificacao2.Text,
      vDataEntrada,
      ...
      DateTimeToStr(now),
      vBloqueio,cbxDestinoAnimal.Selected.Text);
    ...
    begin
     ...
     begin
      if vTipoMovi=0 then
       ...
       StringReplace(edtPeso.Text,',','.',[rfReplaceAll]) ,'CONTROLE',vIdProdutor,'0',vIdLocal,
       FormatDateTime('mm/dd/yyyy',date).QuotedString,frmPrincipal.vIdBaseCampo);
      if vTipoMovi=1 then
       serviceDB.InserePeso(vIDAnimal,serviceDB.vIdUserLogado,
       StringReplace(edtPeso.Text,',','.',[rfReplaceAll]) ,'ENTRADA CONFINAMENTO',vIdProdutor,'0',vIdLocal,
       FormatDateTime('mm/dd/yyyy',date).QuotedString,frmPrincipal.vIdBaseCampo);
     end;
    end;
      ...
      if Switch1.IsChecked then
       begin
         serviceDB.AplicaProtocolo(vidProtocolo, vidAnimal, FormatDateTime('mm/dd/yyyy',date),
          vIdLocal,'1', vIdAplicadorEsquerdo, vIdAplicadorDireito,frmPrincipal.vIdBaseCampo);
       end;
...
begin
 if GridApartacao.Cells[3,GridApartacao.Row].Length>0 then
 begin
   serviceDB.DeletaApartacao(GridApartacao.Cells[3,GridApartacao.Row]);
   GeraGridApartacao(frmPrincipal.vIDPropriedadePropria);
 end;
 serviceDB.qryApartacao.Close;
 serviceDB.qryApartacao.Open;
...
begin
 ...
 begin
   gmd                   := FormatFloat('0.00',(strToFloat(edtGtpUltimo.Text))/
    (Date-strToDate(edtDtUltimoPeso.Text)+1));
...
begin
 serviceDB.AtualizaPesoParte(
  ...
  GridApartacao.Cells[1,GridApartacao.Row],
  GridApartacao.Cells[2,GridApartacao.Row],
  GridApartacao.Cells[4,GridApartacao.Row],
  GridApartacao.Cells[6,GridApartacao.Row]);
  GeraGridApartacao(frmPrincipal.vIDPropriedadePropria);
end;
...
begin
 ...
 if cbxTipoMov.ItemIndex=-1 then
 begin
   MyShowMessage('Informe o tipo de movimentação!',false);
   Exit;
 end;
 ...
 begin
   ...
   serviceDB.TableConfigManejoTIPO_MOV.AsInteger              := cbxTipoMov.ItemIndex;
   vTipoMovi                                                  := cbxTipoMov.ItemIndex;
